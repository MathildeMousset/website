...
- `composite_regularity`: this index is calculated by the SNCF, based on the number of delayed trains, but taking into account the length of the trip. A train is counted on-time if:   
...
tgv_global %>% 
  ...
  stat_summary(fun.y = mean, geom = "line",
               size = 2, colour = "#993350") +
  #geom_line(size = 2, color = "#993350") +
...
Since both rates are similar, I will report only the punctuality from now on, because we know exactly how it is defined.
...
tgv_axes %>% 
  ...
  geom_bar(stat = "identity", fill = "#993350") +
...
```{r heatmap3, fig.height=6, fig.width=10}
tgv_axes %>% 
  ggplot(aes(x = month_english, y = factor(year_ordered, levels=rev(levels(year_ordered))),
             fill = departure_punctuality)) +
  geom_tile() +
