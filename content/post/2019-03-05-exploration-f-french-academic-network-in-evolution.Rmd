...
Some time ago, I donwloaded the summary of all PhD thesis defended in France from 1986 and 2018, as they appear in the national database [these.fr]("these.fr"). One of the national database, at least^[I heard that the file has a slighly different number of lines than the file downloaded from HAL, another national archive, but I cannot find the discussion again].
...
I want the PhDs that were in the evolutionary field. It is a bit tricky because the field name (`discipline` in the dataframe) depends on doctoral schools, and changes every couple of years within doctoral schools.

I first tried filtering by discipline names that contain variation on the word evolution ("Évolut|Evolut|evolut|évolut|") using the `str_detect()` function from the excellent `stringr` package.

I obtained a couple of fields that have nothing to do with evolutionary biology. For example, "evolution of terrestrial systems". Fortunately, the list of fields was not so long, so I could check it and manually filter out the fields that really do not belong (mostly from geology and earth sciences).
